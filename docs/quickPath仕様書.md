...
### 3.3 標準定義コマンド一覧 (現在の設定)

現在、`main.ini` にあらかじめ定義されているカスタムコマンドの説明です。これらは必要に応じて自分用・プロジェクト用にカスタマイズ可能です。

#### `[Commands]` (非同期実行: アプリケーションの起動など)
| キー  | 名称               | 説明                                                                                               |
| :---- | :----------------- | :------------------------------------------------------------------------------------------------- |
| **v** | **VSCODE**         | 選択中のパスを VS Code で開きます。                                                                |
| **a** | **Antigravity**    | 選択中のパスを Antigravity (AIエージェント) で開きます。                                           |
| **b** | **GitBash**        | 選択中のディレクトリを作業ディレクトリとして Git Bash を起動します。                               |
| **m** | **Memorandum**     | PowerShell/Forms を利用したクイックメモ用の GUI ウィンドウを起動します。                           |
| **o** | **AutoWndAlign**   | よく使うアプリ（Antigravity, VS Code, Excel 等）をあらかじめ定義した座標・サイズに自動整列します。 |
| **p** | **ProcessToCsv**   | 実行中のプロセス詳細（PID, メモリ, CPU等）を取得し、指定した CSV ファイルとして出力します。        |
| **w** | **WatchFolder**    | 特定のワークフォルダのファイル変更（作成・削除・リネーム）をリアルタイム監視し、ログを表示します。 |
| **x** | **XlsxToCsv**      | 選択した Excel ファイル (`.xlsx`, `.xls`) の指定シートを CSV 形式に変換し保存します。              |
| **y** | **WatchClipboard** | クリップボードの更新を監視し、履歴を GUI 上で確認・ファイル保存します。                            |

#### `[SyncCommands]` (同期実行: 結果をクリップボードにコピー)
| キー  | 名称           | 説明                                                                                     |
| :---- | :------------- | :--------------------------------------------------------------------------------------- |
| **d** | **Du**         | ディレクトリの使用量を表示します。探索する階層の深さ (Depth) を指定できます。            |
| **u** | **DuFull**     | 全てのサブディレクトリ・ファイルの使用量を再帰的に計算し、CSV 形式で出力します。         |
| **r** | **Tree**       | ディレクトリ構造をツリー形式で出力します。サイズ、作成日、更新日情報を含みます。         |
| **f** | **Find**       | ファイル名を正規表現 (`{{RegExp}}`) で再帰検索し、相対パスを表示します。                 |
| **g** | **Grep**       | ファイル内のテキストを検索 (`{{Search Word}}`) し、一致した行をプレビュー表示します。    |
| **j** | **FindWith7z** | 7-Zip を利用し、圧縮ファイル (zip, 7z 等) の内部も含めてファイル名を検索します。         |
| **k** | **GrepWith7z** | 圧縮ファイル内部のテキストも含めて全文検索を実行します。                                 |
| **l** | **ScoopList**  | Scoop でインストール済みのパッケージ、実行ファイル、インストールパスの一覧を表示します。 |
| **n** | **WindowList** | 現在開いているウィンドウのタイトル、実行プロセス名、座標、サイズを一覧表示します。 |
| **s** | **PingSweep**  | 指定した IP 範囲に対して Ping を送信し、応答のあるホスト情報を一覧表示します。           |
| **i** | **GitStatus**  | 現在のディレクトリで `git status` を実行し、リポジトリの状態を確認します。               |
...
- **1.6.2604 (2026-04-10)**:
  ...
  - カスタムコマンドに `AutoWndAlign (o)` および `WindowList (n)` を追加。ウィンドウ配置の自動化と一覧取得に対応。
- **1.5.2604 (2026-04-07)**:
